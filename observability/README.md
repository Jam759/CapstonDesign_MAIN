...
이 프로젝트는 로컬에서 실행 중인 Spring 애플리케이션의 파일 로그와 메트릭을 수집해 Loki, Prometheus, Grafana에서 조회하는 구성을 포함합니다.
...
- Loki: 로그 저장소
- Prometheus: 로컬 Spring 앱의 `/actuator/prometheus` 메트릭 스크랩
- Grafana: Loki/Prometheus 데이터소스와 기본 대시보드 자동 프로비저닝

## 실행

1. Spring 애플리케이션을 로컬에서 실행합니다.
2. 저장소 루트에서 아래 명령으로 관측 스택만 띄웁니다.

```powershell
docker compose -f .\Docker-Compose.yaml up -d loki alloy prometheus grafana
```

3. 애플리케이션을 재시작해 `/actuator/prometheus` endpoint 노출 설정을 반영합니다.
4. 애플리케이션 요청을 한두 번 보내서 `./.logs` 아래 로그 파일이 생성되도록 합니다.
...
- 구조화 HTTP 로그: `job="capstone-structured-http"`
- 일반 애플리케이션 로그: `job="capstone-application"`
- Prometheus scrape target: `job="capstone-spring"`

## 참고

- Grafana 대시보드는 `Capstone / Capstone Logging Overview`, `Capstone / Capstone Metrics Overview` 로 자동 생성됩니다.
